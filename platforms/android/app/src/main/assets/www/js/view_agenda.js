function view_agenda(retornos){
    ...
    if(retornos['murais']){ 
        for(i  in retorno){
            ...
            conteudo_lista += "<font class='font_text'>"+(retorno[i]['arquivo'] ? "<div id='notimg_"+i+"'><img width='100%' pos='"+i+"' src='./img/carregando.gif'></div><div id='myimg_"+i+"' ><img width='1%'  class='imagens' pos='"+i+"' src='"+(retorno[i]['arquivo']).replace("http://", "https://")+"'></div><br>" : "")+quebraLinha(retorno[i]['mensagem'])+"</font>";
            if(retorno[i]['extras']){
